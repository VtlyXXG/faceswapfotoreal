    ...
    meta.update(
        {
            ...
            # НЕ `mask_px`, и это исправление, а не вкусовщина. `head` здесь —
            # маска по ШАБЛОНУ, тем же вызовом, что `head_old` в transplant.
            # Под именем `mask_px` она затирала одноимённое поле из
            # `pasted.meta`, где лежит размер ОБЪЕДИНЕНИЯ двух масок — той,
            # которой замена и происходит.
            #
            # Цена ошибки была не теоретическая: в мете стояло число, по
            # которому нельзя увидеть, что заменяемая область уехала на
            # туловище. На разборе жалобы «четыре руки» объединение оказалось
            # больше шаблонной маски в трёх бросках из пяти, а мета показывала
            # шаблонную и молчала об этом.
            "template_mask_px": int(np.count_nonzero(head.mask > 127)),
...
def order_done(meta: dict[str, Any]) -> str:
    ...
    ГЕОМЕТРИЯ ЗАМЕНЫ — ЗДЕСЬ ЖЕ, И ЭТО ДОБАВЛЕНО ПО РАЗБОРУ ЖАЛОБЫ. Итоговая
    маска в `transplant` — объединение двух: посчитанной по шаблону и посчитанной
    по ВЫРОВНЕННОЙ ГЕНЕРАЦИИ, а генерация каждый раз новая. Когда вторая
    оказывается крупнее или уезжает сдвигом, заменяемая область уходит с головы
    на туловище, и на кадре появляются руки генерации поверх шаблонных.

    На замере это случилось в трёх бросках из пяти. Разбирали руками полдня;
    с этими четырьмя числами тот же вопрос закрывается сравнением
    `mask_new` с `mask_old` в одной строке журнала.
    """
    return "[order] отдан: " + " ".join(
        ...
        for name, key in (
            ...
            # Три размера подряд: по шаблону, по генерации и то, чем заменяли на
            # самом деле. Порознь они ничего не говорят, сравнение — говорит
            ("mask_old", "mask_old_px"),
            ("mask_new", "mask_new_px"),
            ("mask_used", "mask_px"),
            # Куда и во сколько раз уехала голова при подгонке. Большой сдвиг
            # утаскивает маску за собой даже когда она сама не выросла
            ("shift", "shift"),
            ("scale", "scale"),
