...
# Пределы показателя гаммы, которой правится светлота кожи. Выход за них
# означает, что мерка попала не на кожу — например, разметка приняла за кожу
# песок или ткань, — и подгонка по такому числу перекрасила бы лицо в цвет
# ошибки.
...
def _match_skin(
    ...
    # Гамма считается по долям от максимума светлоты: медиана донора обязана
    # сесть на медиану персонажа, а концы шкалы остаться на местах. Логарифмы
    # берутся от чисел строго внутри (0, 1) — на самих концах гамма не
    # определена, и кожа там означает, что мерка попала на пересвет или в тень
    top = 255.0
    src_share = float(np.clip(src[0] / top, 0.02, 0.98))
    dst_share = float(np.clip(dst[0] / top, 0.02, 0.98))
    gamma = float(np.clip(np.log(dst_share) / np.log(src_share),
    ...
    corrected = lab.copy()
    corrected[..., 0] = top * np.power(np.clip(lab[..., 0] / top, 0.0, 1.0), gamma)
    corrected[..., 1:] += shift
