...
class Settings(BaseSettings):
    ...
    # Стирать ли причёску нарисованного персонажа там, где её не закрыла
    # вклейка. Без этого у длинноволосого героя вокруг вклеенной головы
    # остаётся его собственная шевелюра. Дыра затягивается локально (Telea) и
    # попадает в маску инпейнтинга; выключать имеет смысл, только если
    # сегментатор раз за разом промахивается по конкретной серии обложек.
