...
различим. Это задача диффузионного стилизатора (ниже).

### Диффузионный стилизатор: SD 1.5 + IP-Adapter FaceID

`ML_STYLE_PROVIDER=diffusion` включает ре-синтез лица под стиль обложки: img2img
поверх перенесённого лица, идентичность держит IP-Adapter FaceID.

Ключевые решения:

- **Идентичность передаётся вектором, а не картинкой.** IP-Adapter FaceID
  принимает тот самый 512-мерный ArcFace-эмбеддинг донора, что уже отдаёт
  детектор (`w600k_r50`) — ни CLIP-энкодер, ни второй проход не нужны. Форма
  тензора `(2, 512)` продиктована diffusers 0.30 (`chunk(2)` на негатив/позитив).
- **Зона идентичности берёт меньше диффузии.** В области глаз/носа/рта
  подмешивание урезано на `ML_STYLE_DIFFUSION_IDENTITY_KEEP`, чтобы не сдвигать
  геометрию — она и есть узнаваемость.
- **negative важнее positive.** Борьба идёт с фотографичностью
...
```powershell
.\.venv\Scripts\python.exe scripts\fetch_style_models.py
```

Кладёт SD 1.5 в `models/sd15/` и IP-Adapter FaceID в `models/ip-adapter-faceid/`.

> **CPU против GPU.** На `device: cpu` одна диффузионная стадия — это минуты на
> лицо. Для отладки масок и параметров это терпимо, для продакшена нужна GPU
> (на `cuda` стилизатор сам берёт float16). Параметры — `ML_STYLE_DIFFUSION_*`
> в `.env.example`; сравнить с classical по метрикам — `python -m bench --diffusion`.
...
app/
  ...
  pipelines/
    ...
    style/
      ...
      diffusion.py      SD 1.5 + IP-Adapter FaceID (нужны веса)
      faceid.py         ArcFace-эмбеддинг → вход IP-Adapter FaceID
      prompts.py        промпты диффузии из стиля обложки
...
Node-сервис ходит на `http://localhost:8000`. Готовность этого сервиса —
готовность всего продукта: замена лица и есть его единственная работа,
