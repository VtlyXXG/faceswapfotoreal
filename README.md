...
с его лицом. Вся тяжёлая работа живёт в Python-частях:

- `ml-service/` — геометрия: маска головы персонажа, семантическая разметка,
  выравненный кроп донора и **пересадка головы** (`transplant.py`);
- `gpu-inference-server/` — генерация на своей карте: **FLUX.2** рисует кадр с
  новой головой, **LaMa** стирает старую по силуэту.

Всё, что вне головы, возвращается из шаблона побитово — разворот нарисован
художником и обязан дойти до печати без изменений. Подробности в
`ml-service/README.md` и `gpu-inference-server/README.md`.

**Стороннего платного сервиса не требуется, и ключей для запуска не нужно
никаких.** Путь через fal.ai остался в коде, но выключен настройкой
(`ML_FAL_ENABLED=false`) и ключа не спрашивает.
...
ml-service/             Python (FastAPI): разметка, маска головы, пересадка головы
gpu-inference-server/   Python (FastAPI) на GPU-боксе: FLUX.2, LaMa, SDXL-путь
tools/                  стендовые мерки качества пересадки — см. tools/README.md
tests/                  node:test
```
...
Python ML-сервис по HTTP и пробрасывает `X-Request-ID`. Детекция лиц, разметка,
маска головы и пересадка настраиваются на стороне `ml-service/` (см. его README
и `ml-service/.env.example`); тяжёлые веса — FLUX.2, LaMa, SDXL — лежат только на
GPU-боксе и настраиваются переменными `GPU_*` (см.
`gpu-inference-server/README.md`).
