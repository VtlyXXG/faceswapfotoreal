...
с его лицом. Вся тяжёлая работа живёт в Python ML-сервисе (`ml-service/`): лицо
переносится локально жёстким коллажем, а стилизует его инпейнтинг на fal.ai —
подробности в `ml-service/README.md`. Node ничего не генерирует сам: он
принимает заказы, ведёт их состояние и оркестрирует вызовы ML.
...
ml-service/             Python (FastAPI): детекция, коллаж лица, стилизация
tests/                  node:test
```
...
детекции лиц, переноса лица и стилизации, настраивается на стороне
